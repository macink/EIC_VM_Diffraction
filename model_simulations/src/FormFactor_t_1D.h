...
public:
        ...
        double t_min, double t_max, double bins, double r_min, double r_max): A(A_init), Vo(Vo_init), R(R_init), a0(a0_init)
    {
       ...
       // Initialize hist for transform: |F(t)|^2 -> G(r)
        transform_hist = new TH1D("","", 2*bins, -r_max, r_max);
        ...
        */
        double step1 = 2*r_max / (2*bins);  // step from -r_max to r_max
        for (int i = 0; i < 2*bins; ++i)
        {
            double r = -r_max + (i + 0.5)*step1;  // center of each bin
            double val = trueG1_integral_dq_1D(fabs(r)); // use symmetry: G(r) = G(-r)
            transform_hist->SetBinContent(i+1, val);
        }
        double ws_integral = transform_hist->Integral();
		// normalize
		if(ws_integral) 
		{
    		transform_hist->Scale(1/ws_integral);
		}
