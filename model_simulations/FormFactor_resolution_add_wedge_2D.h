...
public:
        ...
        double x_max, double y_min, double y_max): A(A_init), Vo(Vo_init), R(R_init), a0(a0_init) 
    {
        ...
        cutFFt2d = new TF2("", [this, phi_min, phi_max, sigma] (double *var, double *par)
        {
            double tx_prime = var[0], ty = var[1]; 
            TF1 fft2("", FF_cut_wRes_2D, phi_min, phi_max, 3);
        ...
        hist2D = new TH2D("", "Form Factor", bins, tx_prime_min, tx_prime_max, bins, ty_min, ty_max);
        hist2D->Sumw2();
    ...
    static double wedge_FF_2D(double tx_prime, double ty, double sigma, double phi_min, double phi_max)
    {
        ...
        f->SetParameters(tx_prime, ty, sigma);
...
public:
        ...
        double x_max, double y_min, double y_max): A(A_init), Vo(Vo_init), R(R_init), a0(a0_init) 
    {
        ...
        // Initialize histogram for TF2
        hist2D = new TH2D("", "", bins, tx_prime_min, tx_prime_max, bins, ty_min, ty_max);
        hist2D->Sumw2();
        double step2 = tx_prime_max/bins;
        for(int i=0;i<bins;i++) 
        {
    ...
    {
        //double qx_prime = sqrt(tx_prime), qy = sqrt(qy);
        TF1 *f = new TF1("",FF_cut_wRes_2D,phi_min,phi_max,3);  
        f->SetParameters(tx_prime, ty, sigma);
        return f->Integral(phi_min,phi_max,1e-12); //[GeV]
    }
