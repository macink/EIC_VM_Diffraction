...
public:
        ...
        double qx_max, double bins): A(A_init), Vo(Vo_init), R(R_init), a0(a0_init)
    {
        ...
        FF2DqSquared = new TF2("Form Factor", calc_FormFactor_2q2D, -qx_max, qx_max, -qy_max, qy_max, 4);
        FF2DqSquared->SetParameters(A, Vo, R, a0);

        // Initialize |F(qx,qy)^2| 2D histogram
        hist2D = new TH2D("", "|F(q_{x},q_{y})^{2}| 2D Histogram", bins, -qx_max, qx_max, bins, -qy_max, qy_max);
        hist2D->Sumw2();
