...
void combine_histograms(TString inputDir, TString outputFile) 
{
    ...
    TH1D* h_t_REC_new_method_combined = nullptr;
    ...
    for(const auto& fileName : fileNames) 
    {
        ...
        TH1D* h_t_REC_new_method = dynamic_cast<TH1D*>(inputFile->Get("h_t_REC_new_method"));
        ...
        if(h_t_REC_new_method) 
        {
            if(!h_t_REC_new_method_combined) 
            {
                h_t_REC_new_method_combined = dynamic_cast<TH1D*>(h_t_REC_new_method->Clone("h_t_REC_new_method_combined"));
                h_t_REC_new_method_combined->SetDirectory(nullptr);
            } 
            else 
            {
                h_t_REC_new_method_combined->Add(h_t_REC_new_method);
            }
        }
        delete inputFile;
    }
    ...
    if(h_t_REC_new_method_combined) 
    {
        cout << "Saving combined histogram: h_t_REC_new_method" << endl;
        h_t_REC_new_method_combined->Write();
    } 
    else 
    {
        cout << "Error: Combined histogram h_t_REC_new_method is null!" << endl;
    }
    
    if(h_t_MC_combined) h_t_MC_combined->Write();
    if(h_t_REC_combined) h_t_REC_combined->Write();
    if(h_t_REC_new_method_combined) h_t_REC_new_method_combined->Write();
