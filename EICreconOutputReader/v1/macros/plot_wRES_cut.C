...
void plot_wRES_cut(TString filename="./output/eicrecon-sartre_coherent_phi_output.root")
{	
	...
	TH1D* h_t_REC_wRES_cut = (TH1D*) file->Get("h_t_REC_wRES_cut");
	...
	if(integral_MC>0 && integral_REC>0 && integral_REC_wRES_cut>0) 
	{
    	h_t_REC->Scale(integral_MC/integral_REC);
    	h_t_REC_wRES_cut->Scale(integral_MC/integral_REC_wRES_cut);
	}
	...
	w7->Draw("same");
	c1->Print("./figures/tests/wRES_cut.pdf");
}
