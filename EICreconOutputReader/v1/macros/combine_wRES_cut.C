...
void combine_wRES_cut(TString inputDir, TString outputFile) 
{
    ...
    TH1D* h_t_REC_wRES_cut_combined = nullptr;
    ...
    for(const auto& fileName : fileNames) 
    {
        ...
        TH1D* h_t_REC_wRES_cut = dynamic_cast<TH1D*>(inputFile->Get("h_t_REC_wRES_cut"));
        ...
        if(h_t_REC_wRES_cut) 
        {
            if(!h_t_REC_wRES_cut_combined) 
            {
                h_t_REC_wRES_cut_combined = dynamic_cast<TH1D*>(h_t_REC_wRES_cut->Clone("h_t_REC_wRES_cut_combined"));
                h_t_REC_wRES_cut_combined->SetDirectory(nullptr);
            } 
            else 
            {
                h_t_REC_wRES_cut_combined->Add(h_t_REC_wRES_cut);
            }
        }
        delete inputFile;
    }
    ...
    if(h_t_REC_wRES_cut_combined) 
    {
        cout << "Saving combined histogram: h_t_REC_wRES_cut" << endl;
        h_t_REC_wRES_cut_combined->Write();
    } 
    else 
    {
        cout << "Error: Combined histogram h_t_REC_wRES_cut is null!" << endl;
    }
    
    if(h_t_MC_combined) h_t_MC_combined->Write();
    if(h_t_REC_combined) h_t_REC_combined->Write();
    if(h_t_REC_wRES_cut_combined) h_t_REC_wRES_cut_combined->Write();
