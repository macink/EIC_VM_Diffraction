...
public:
        ...
        double ty_min, double ty_max, double bins, double x_min, double x_max, double y_min, double y_max): A(A_init), Vo(Vo_init), R(R_init), a0(a0_init)
    {
        ...
        transform_TF2 = new TF2("Fourier-Bessel Transformation: |F(t_{x},t_{y})|^{2} -> G(x,y)", [this, tx_min, tx_max, ty_min, ty_max] (double *var, double *par)
        {
            double x = var[0], y = var[1];
            TF2 trans_integral("trans_integral", trueG1_integrand_dq_2D, tx_min, tx_max, ty_min, ty_max, 2);
        ...
        transform_hist = new TH2D("hankelTransformFF", "Transform Form Factor", bins, x_min, x_max, bins, y_min, y_max);
