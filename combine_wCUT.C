...
void combine_wCUT(TString inputDir, TString outputFile) 
{
    ...
    TH1D* h_t_REC_wCUT_combined = nullptr;
    ...
    for(const auto& fileName : fileNames) 
    {
        ...
        TH1D* h_t_REC_wCUT = dynamic_cast<TH1D*>(inputFile->Get("h_t_REC_wCUT"));
        ...
        if(h_t_REC_wCUT) 
        {
            if(!h_t_REC_wCUT_combined) 
            {
                h_t_REC_wCUT_combined = dynamic_cast<TH1D*>(h_t_REC_wCUT->Clone("h_t_REC_wCUT_combined"));
                h_t_REC_wCUT_combined->SetDirectory(nullptr);
            } 
            else 
            {
                h_t_REC_wCUT_combined->Add(h_t_REC_wCUT);
            }
        }
        delete inputFile;
    }
    ...
    if(h_t_REC_wCUT_combined) 
    {
        cout << "Saving combined histogram: h_t_REC_wCUT" << endl;
        h_t_REC_wCUT_combined->Write();
    } 
    else 
    {
        cout << "Error: Combined histogram h_t_REC_wCUT is null!" << endl;
    }
    
    if(h_t_MC_combined) h_t_MC_combined->Write();
    if(h_t_REC_combined) h_t_REC_combined->Write();
    if(h_t_REC_wCUT_combined) h_t_REC_wCUT_combined->Write();
