...
{
public:
    FormFactor_resolution_add_wedge_1D2(TLorentzVector vmMC_init, double A_init, double Vo_init, double R_init, double a0_init, double t_min, double t_max, 
        double bins, double phi_min, double phi_max, double sigma): A(A_init), Vo(Vo_init), R(R_init), a0(a0_init) 
    {
        ...
        double tx = vmMC.Px()*vmMC.Px(); 
        double ty = vmMC.Py()*vmMC.Py(); 
        double t = tx+ty;
